// https://leetcode.com/problems/add-two-numbers/
// You are given two non-empty linked lists representing two non-negative integers. The digits are stored in reverse order, and each of their nodes contains a single digit.
// Add the two numbers and return the sum as a linked list.
// You may assume the two numbers do not contain any leading zero, except the number 0 itself.

// Example 1:
// Input: l1 = [2,4,3], l2 = [5,6,4]
// Output: [7,0,8]
// Explanation: 342 + 465 = 807.
// Example 2:

// Input: l1 = [0], l2 = [0]
// Output: [0]
// Example 3:

// Input: l1 = [9,9,9,9,9,9,9], l2 = [9,9,9,9]
// Output: [8,9,9,9,0,0,0,1]

// This ignores the LinkedList prompt and simply tries an O(n) approach to the prompt via JS arrays.
// If each integer was a node instead, we can simply use node.val for access and push the "remainder" to node.next to increment its value.
// Then we wouldn't need the remainder variable, but would need to create a new node when the iteration is complete and we have a "remainder" left over (1 in case of a 10 value).

import LinkedList, {ListNode, linkedListToArray} from "../../helpers/linked-list";

interface TSNode {
  val: any;
  next: TSNode | null;
};

const addTwoNumbers = (l1: number[], l2: number[]) => {
  const L1 = (LinkedList(l1) as unknown) as TSNode | null;
  const L2 = (LinkedList(l2) as unknown) as TSNode | null;
  const RList = ListNode(null) as TSNode;

  let head = RList;
  let list1 = L1;
  let list2 = L2;
  let remainder = 0;

  while (list1 !== null || list2 !== null) {
    const a = list1?.val ?? 0;
    const b = list2?.val ?? 0;
    const sum = a + b + remainder;

    const val = sum > 9 ? sum % 10 : sum;
    remainder = sum > 9 ? 1 : 0;

    list1 = list1 !== null ? list1.next : null;
    list2 = list2 !== null ? list2.next : null;

    head.next = ListNode(val) as TSNode;
    head = head.next;
  }

  if (remainder > 0) {
    head.next = ListNode(remainder) as TSNode;
  }

  return linkedListToArray(RList.next);
};

export default addTwoNumbers;
